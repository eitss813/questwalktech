...
<!-- <?php // echo "<pre>"; print_r($userGamesInCart); die(' here '); ?> -->
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
              <button type="button" title="Purchase all cards" class="btn btn-outline-success float-right" id="purchaseAllGame" onclick="purchaseAllGame();" style="background: #28a745; color: #fff;">Buy/ Get For Free</button>
          </div>
          <!-- <div class="col-sm-6">
  ...
  <script>
    ...
    function purchaseAllGame()
    {
      ...
      var tmpPrize = [];
      var gameidsToSend = '';
      // alert(cartGames.length);
      if(cartGames.length > 0)
      {
        $.each(cartGames, function(i,e){
          var image_game = cartGames[i].Game_Image ? cartGames[i].Game_Image : 'Game2.jpg'; // if there is no image, set default image
          var Game_prise = cartGames[i].Game_prise ? (cartGames[i].Game_prise - cartGames[i].Game_discount) : '0.00'; // if there is no prise, set 0.00
          showHtml += "<div class='row col-md-12 mb-3' style='min-width:90vh;'><div class='col-md-8'><img src='<?php echo API_URL;?>../../images/"+image_game+"' style='max-width:100px;'></img> &nbsp;"+cartGames[i].Game_Name+"</div> <div class='col-md-4'>&#8377; "+Game_prise+"</div></div>";
        ...
        swalWithBootstrapButtons.fire({
          ...
          if(response.value == true)
          {
            ...
            if($('#amount').val() < 1)
            {
              ...
              window.location = "<?php echo base_url('Payment/freeGame/');?>"+gameidsToPass/'free';
            }
            else
            {
