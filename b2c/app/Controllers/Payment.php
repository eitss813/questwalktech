...
class Payment extends BaseController
{
  ...
  public function success($arg=NULL)
  {
    ...
    if($RequestMethod == 'POST')
    {
      ...
      $contant['imgUrl'] =  'writable/uploads/please_wait.gif';
  ...
  public function freeGame($freeImg=NULL, $uid=NULL, $arg=NULL)
  {
    // $uid = $this->session->get('loginUserData')->user_Id;
    // show this page when payement is made successfully
    // $RequestMethod = $_SERVER['REQUEST_METHOD'];
    if(!empty($arg))
    {
      if(empty(session('loginUserData')))
      {
        $setSessionData = $this->CommonModel->executeQuery("SELECT * FROM game_users WHERE user_Id=$uid");
        // echo "<pre>"; print_r($setSessionData['userData'][0]); exit(); 
        $this->session->set('loginUserData',$setSessionData['userData'][0]);
      }
      // echo "<pre>$arg<br>".trim(base64_decode($arg),',')."<br>"; print_r($_POST); exit();
      // txnStatus txnid MerTxnId // $_POST['status'] $_POST['txnid'] $_POST['mihpayid']
      $contant['imgUrl'] =  'writable/uploads/please_wait.gif';
  ...
  private function checkLogin($postData)
  {
    if(empty(session('loginUserData')))
    {
      $user_Email = $postData['email'];
      $dbResult = $this->CommonModel->verifyLogin($user_Email, $user_Email);
      $this->session->set('loginUserData',$dbResult['userData'][0]);
      // echo "Session Expired, so setting the session.<br>";
    }
    // echo "<pre>"; print_r(session('loginUserData')); print_r($postData); exit();
  }
